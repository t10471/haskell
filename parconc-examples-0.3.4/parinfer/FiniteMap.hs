...
instance (Show a, Show b) => Show (FM a b) where
      showsPrec d  =  showsFM
showsFM              :: (Show a, Show b) => Shows (FM a b)
showsFM              =  showsSurround "[" (showsStarSep ",\n " showsPair) "]"
                      .  unmakeFM
showsPair             :: (Show a, Show b) => Shows (a, b)
showsPair (x,t)       =  showsString (show(x)) . showsString " : " . shows t
