...
-- <<search_type
search :: ( partial -> Maybe solution )   -- <1> 部分解が完全解調べ、完全解を返す(違えば、Nothing)
       -> ( partial -> [ partial ] )      -- <2> 部分解を受け取り、一歩進めた部分解のリストを返す
       -> partial                         -- <3> 部分解の初期値(空の値)
       -> [solution]                      -- <4> 完全解のリスト
-- >>
...
search finished refine emptysoln = generate emptysoln
  where
    -- これが再帰する
    generate partial
      | Just soln <- finished partial = trace "Just soln" [soln]
      -- refine
      -- partialで配列を返しmapでgenerateを適用再帰し、結果[soln]をconcatする
...
timetable people allTalks maxTrack maxSlot =
 ...
 where
  ...
  refine (slotNo, trackNo, slots, slot, slotTalks, talks)
     | trackNo == maxTrack = trace ("max   " ++ show (dd)) dd 
     | otherwise = trace ("no max" ++ show (d)) d
    where
      d :: [Partial]
      d = 
      ...
      dd :: [Partial]
      dd = [(slotNo+1, 0, slot:slots, [], talks, talks)]
-- >>
