...
-- なんだか本物と違うきがする
-- 本物は concurrently を使った関数合成になっている
-- <<Functor
...
waitBoth a1 a2 =
  atomically $ do
    -- a2 の例外を拾うために a1 で retry が発生したら a2 を実施するために
    -- 1行で両方の処理を記述している
    -- ただし、a2 の結果は子の行では捨てる
    --   waitSTM はエラーが発生すると例外をなげる
    --   空ならば retry する
    --   retry すろと orElse は 右側を実行する
    -- a2 の結果が帰ってきたら a1 を待つため retry する
