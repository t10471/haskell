...
-- <<update
  update g k = runPar $ do
    -- spawnはputしてforkする処理
    -- Map.traverseWithKey :: 
    --   (Key -> IntMap Weight -> Par (IVar (IntMap Weight)))
    --     -> IntMap (IntMap Weight) 
    --     -> Par (IntMap (IVar (IntMap Weight)))
    m <- Map.traverseWithKey (\i jmap -> spawn (return (shortmap i jmap))) g
    -- traverse :: 
    --   (IVar (IntMap Weight) -> Par (IntMap Weight)) 
    --     -> IntMap (IVar (IntMap Weight)) 
    --     -> Par (IntMap (IntMap Weight))
    traverse get m
-- >>
   where
