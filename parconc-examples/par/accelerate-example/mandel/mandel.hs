...
mandelbrot x y x' y' screenX screenY max_depth
  -- iterate した結果から limit (max_depth) を取得
...
genPlane :: F -> F
         ...
         -> Acc ComplexPlane
-- >>
-- generate は シェイプのインデックスを関数に渡す
-- viewy = 3, viewx = 5 なら pr.x は 0 から 4 pr.y は 0 から 2
genPlane lowx lowy highx highy viewx viewy
   = generate (constant (Z:.viewy:.viewx))
              -- unindex2 :: Exp ((Z :. Int ) :. Int) -> Exp (Int, Int)
...
iter c p =
  ...
  in
  -- 2乗して4を越えると発散する
  -- 本当は発散したら処理をやめたいができないので取得した値をもう一度設定する
  (dot z' >* 4.0) ?                                -- <3>
     ( p                                           -- <4>
     , lift (z', i+1)                              -- <5>
     )
...
main
  ...
  = do  (config, nops) <- processArgs =<< getArgs
        let size        = optSize config
            ...
            y'          = -0.75
            --
            -- config は CUDAを使うかどうかを判定するために持ちまわる
