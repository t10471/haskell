...
bestKey tab = fst $ List.maximumBy 
                    (\(_,s) (_,s') -> s `compare` s') 
                    $ fmap (\k -> (k, maxOf (scores k tab))) 
                           (keys tab)
    where maxOf (x, y) = max x y
...
discriminate tab
  ...
  | otherwise
    = let key          = bestKey tab
          (tTab, fTab) = filterOn key tab
       in Left $ \row ->
          let bool = getKey key row
           in if bool then tTab else fTab
...
predict' row model = cataF phi model
  where phi br = case getKey (key br) row of
                 True  -> bTrue  br
                 False -> bFalse br
